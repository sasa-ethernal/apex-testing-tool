...
# TODO be smart about UTXO?? check if amount is sufficient
...
cardano-cli transaction build-raw \
    ...
    --invalid-hereafter 0 \
    --fee 0 \
    --out-file tx_${TX_TIME}.draft

# Calculate fee
FEE=$(cardano-cli transaction calculate-min-fee \
    --tx-body-file tx_${TX_TIME}.draft \
    --tx-in-count 1 \
    --tx-out-count 2 \
    --witness-count 1 \
...
cardano-cli transaction sign \
    ...
    --tx-body-file tx_${TX_TIME}.draft \
    --out-file tx_${TX_TIME}.signed \
    ${CARDANO_NET_PREFIX}

echo "$(cardano-cli transaction submit \
    --tx-file tx_${TX_TIME}.signed \
    ${CARDANO_NET_PREFIX})"
